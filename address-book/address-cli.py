...
def list_contacts(filepath):
    ...
    if addressbook:
        for contact in addressbook['contacts']:
            print('Name: ' + contact['name'], 'Telephone: ' + contact['phone'],
                  'Address: ' + contact['address'], sep='\t', end='\n')
    else:
    ...
    if fullname:
        ...
        if addressbook is None:
            ...
            addressbook = {'filetype': 'Addressbook',
                           'application': 'address-cli.py', 'contacts': contacts}
...
def process_options(argv, inputfile):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hi:lar:f:", ["help", "ifile=", "list", "add", "remove", "find="])
